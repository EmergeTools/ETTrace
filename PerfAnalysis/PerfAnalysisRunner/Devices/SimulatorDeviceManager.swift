...
struct SimulatorDeviceManager: DeviceManager {
    ...
    func copyFromDevice(bundleId: String, source: String, destination: String) throws {
        ...
        let dstFull = "\(destination)/\(sourceURL.deletingLastPathComponent!.path)"
    ...
    func currentHomeDir(_ bundleId: String) throws -> String {
        return try safeShellWithOutput("/usr/bin/xcrun simctl get_app_container \(deviceUuid) \(bundleId) data").trimmingCharacters(in: .whitespacesAndNewlines)
    }
